...
def pairs_by_dist_np(N, distances):
    """Сортирует пары вершин по возрастанию расстояния между ними"""
    pairs = np.zeros((N * (N - 1) // 2,), dtype=('f4, i2, i2'))
...
def solve_tsp(distances, optim_steps=3, pairs_by_dist=pairs_by_dist_np):
    ...
    def join_segments(sorted_pairs):
        ...
        for i, j in islice(filtered_pairs(), N - 1):
            ...
            seg_i.extend(seg_j)
        # результат работы записан в connections
...
def main():
    ...
    img = mpli.imread('1.png')
    pp.imshow(img)
    pp.plot(xy[0, path], xy[1, path], 'o-')
    pp.show()
